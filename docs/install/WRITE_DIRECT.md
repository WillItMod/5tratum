...
2. Select `5tratumos-installer-v0.3.113.iso` (you may need to choose "*.* / All files").
...
```sh
sudo dd if=5tratumos-installer-v0.3.113.iso of=/dev/sdX bs=8M status=progress oflag=sync
sync
```
...
sudo dd if=5tratumos-installer-v0.3.113.iso of=/dev/rdiskN bs=8m
sync
```
